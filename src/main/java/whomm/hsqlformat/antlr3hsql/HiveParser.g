...
explainStatement
@init { pushMsg(retval.start,"explain statement", state); }
@after { popMsg(state); }
	: KW_EXPLAIN (
...
vectorizationOnly
@init { pushMsg(retval.start,"vectorization's only clause", state); }
@after { popMsg(state); }
    : KW_ONLY
    -> ^(TOK_ONLY)
    ;
...
vectorizatonDetail
@init { pushMsg(retval.start,"vectorization's detail level clause", state); }
@after { popMsg(state); }
    : KW_SUMMARY
    -> ^(TOK_SUMMARY)
    | KW_OPERATOR
...
execStatement
@init { pushMsg(retval.start,"statement", state); }
@after { popMsg(state); }
...
loadStatement
@init { pushMsg(retval.start,"load statement", state); }
@after { popMsg(state); }
...
replicationClause
@init { pushMsg(retval.start,"replication clause", state); }
@after { popMsg(state); }
...
exportStatement
@init { pushMsg(retval.start,"export statement", state); }
@after { popMsg(state); }
    : KW_EXPORT
...
importStatement
@init { pushMsg(retval.start,"import statement", state); }
@after { popMsg(state); }
       : KW_IMPORT
...
replDumpStatement
@init { pushMsg(retval.start,"replication dump statement", state); }
@after { popMsg(state); }
      : KW_REPL KW_DUMP
...
replLoadStatement
@init { pushMsg(retval.start,"replication load statement", state); }
@after { popMsg(state); }
      : KW_REPL KW_LOAD
        ((dbName=identifier) (DOT tblName=identifier)?)?
        KW_FROM (path=StringLiteral)
...
replConfigs
@init { pushMsg(retval.start,"repl configurations", state); }
@after { popMsg(state); }
    :
...
replConfigsList
@init { pushMsg(retval.start,"repl configurations list", state); }
@after { popMsg(state); }
    :
...
replStatusStatement
@init { pushMsg(retval.start,"replication status statement", state); }
@after { popMsg(state); }
      : KW_REPL KW_STATUS
        (dbName=identifier) (DOT tblName=identifier)?
        (KW_WITH replConf=replConfigs)?
...
ddlStatement
@init { pushMsg(retval.start,"ddl statement", state); }
@after { popMsg(state); }
...
ifExists
@init { pushMsg(retval.start,"if exists clause", state); }
@after { popMsg(state); }
    : KW_IF KW_EXISTS
    -> ^(TOK_IFEXISTS)
    ;
...
restrictOrCascade
@init { pushMsg(retval.start,"restrict or cascade clause", state); }
@after { popMsg(state); }
    : KW_RESTRICT
    -> ^(TOK_RESTRICT)
    | KW_CASCADE
...
ifNotExists
@init { pushMsg(retval.start,"if not exists clause", state); }
@after { popMsg(state); }
...
rewriteEnabled
@init { pushMsg(retval.start,"rewrite enabled clause", state); }
@after { popMsg(state); }
...
rewriteDisabled
@init { pushMsg(retval.start,"rewrite disabled clause", state); }
@after { popMsg(state); }
...
storedAsDirs
@init { pushMsg(retval.start,"stored as directories", state); }
@after { popMsg(state); }
...
orReplace
@init { pushMsg(retval.start,"or replace clause", state); }
@after { popMsg(state); }
    : KW_OR KW_REPLACE
    -> ^(TOK_ORREPLACE)
    ;
...
createDatabaseStatement
@init { pushMsg(retval.start,"create database statement", state); }
@after { popMsg(state); }
...
dbLocation
@init { pushMsg(retval.start,"database location specification", state); }
@after { popMsg(state); }
    :
...
dbProperties
@init { pushMsg(retval.start,"dbproperties", state); }
@after { popMsg(state); }
    :
...
dbPropertiesList
@init { pushMsg(retval.start,"database properties list", state); }
@after { popMsg(state); }
    :
...
switchDatabaseStatement
@init { pushMsg(retval.start,"switch database statement", state); }
@after { popMsg(state); }
    : KW_USE identifier
...
dropDatabaseStatement
@init { pushMsg(retval.start,"drop database statement", state); }
@after { popMsg(state); }
...
databaseComment
@init { pushMsg(retval.start,"database's comment", state); }
@after { popMsg(state); }
...
createTableStatement
@init { pushMsg(retval.start,"create table statement", state); }
@after { popMsg(state); }
...
truncateTableStatement
@init { pushMsg(retval.start,"truncate table statement", state); }
@after { popMsg(state); }
...
dropTableStatement
@init { pushMsg(retval.start,"drop statement", state); }
@after { popMsg(state); }
...
alterStatement
@init { pushMsg(retval.start,"alter statement", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"alter table statement", state); }
@after { popMsg(state); }
...
@init {pushMsg(retval.start,"alter table partition statement suffix", state);}
@after {popMsg(state);}
...
@init { pushMsg(retval.start,"alter view statement", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"alter materialized view statement", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"alter database statement", state); }
@after { popMsg(state); }
...
alterDatabaseSuffixProperties
@init { pushMsg(retval.start,"alter database properties statement", state); }
@after { popMsg(state); }
...
alterDatabaseSuffixSetOwner
@init { pushMsg(retval.start,"alter database set owner", state); }
@after { popMsg(state); }
...
alterDatabaseSuffixSetLocation
@init { pushMsg(retval.start,"alter database set location", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"rename statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixAddCol
@init { pushMsg(retval.start,"add column statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixAddConstraint
@init { pushMsg(retval.start,"add constraint statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixUpdateColumns
@init { pushMsg(retval.start,"update columns statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixDropConstraint
@init { pushMsg(retval.start,"drop constraint statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixRenameCol
@init { pushMsg(retval.start,"rename column name", state); }
@after { popMsg(state); }
...
alterStatementSuffixUpdateStatsCol
@init { pushMsg(retval.start,"update column statistics", state); }
@after { popMsg(state); }
...
alterStatementSuffixUpdateStats
@init { pushMsg(retval.start,"update basic statistics", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"add partition statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixTouch
@init { pushMsg(retval.start,"touch statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixArchive
@init { pushMsg(retval.start,"archive statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixUnArchive
@init { pushMsg(retval.start,"unarchive statement", state); }
@after { popMsg(state); }
...
partitionLocation
@init { pushMsg(retval.start,"partition location", state); }
@after { popMsg(state); }
    :
...
@init { pushMsg(retval.start,"drop partition statement", state); }
@after { popMsg(state); }
...
alterStatementSuffixProperties
@init { pushMsg(retval.start,"alter properties statement", state); }
@after { popMsg(state); }
...
alterViewSuffixProperties
@init { pushMsg(retval.start,"alter view properties statement", state); }
@after { popMsg(state); }
    : KW_SET KW_TBLPROPERTIES tableProperties
...
alterMaterializedViewSuffixRewrite
@init { pushMsg(retval.start,"alter materialized view rewrite statement", state); }
@after { popMsg(state); }
...
alterMaterializedViewSuffixRebuild
@init { pushMsg(retval.start,"alter materialized view rebuild statement", state); }
@after { popMsg(state); }
    : KW_REBUILD -> ^(TOK_ALTER_MATERIALIZED_VIEW_REBUILD)
    ;

alterStatementSuffixSerdeProperties
@init { pushMsg(retval.start,"alter serdes statement", state); }
@after { popMsg(state); }
...
@init {pushMsg(retval.start,"table partition prefix", state);}
@after {popMsg(state);}
...
alterStatementSuffixFileFormat
@init {pushMsg(retval.start,"alter fileformat statement", state); }
@after {popMsg(state);}
...
alterStatementSuffixClusterbySortby
@init {pushMsg(retval.start,"alter partition cluster by sort by statement", state);}
@after {popMsg(state);}
...
alterTblPartitionStatementSuffixSkewedLocation
@init {pushMsg(retval.start,"alter partition skewed location", state);}
@after {popMsg(state);}
...
skewedLocations
@init { pushMsg(retval.start,"skewed locations", state); }
@after { popMsg(state); }
    :
...
skewedLocationsList
@init { pushMsg(retval.start,"skewed locations list", state); }
@after { popMsg(state); }
    :
...
skewedLocationMap
@init { pushMsg(retval.start,"specifying skewed location map", state); }
@after { popMsg(state); }
    :
...
alterStatementSuffixLocation
@init {pushMsg(retval.start,"alter location", state);}
@after {popMsg(state);}
...
alterStatementSuffixSkewedby
@init {pushMsg(retval.start,"alter skewed by statement", state);}
@after{popMsg(state);}
	: tableSkewed
...
alterStatementSuffixExchangePartition
@init {pushMsg(retval.start,"alter exchange partition", state);}
@after{popMsg(state);}
...
alterStatementSuffixRenamePart
@init { pushMsg(retval.start,"alter table rename partition statement", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"alter table stats partition statement", state); }
@after { popMsg(state); }
    : KW_UPDATE KW_STATISTICS KW_FOR KW_COLUMN? colName=identifier KW_SET tableProperties (KW_COMMENT comment=StringLiteral)?
    ->^(TOK_ALTERTABLE_UPDATECOLSTATS $colName tableProperties $comment?)
    ;
...
alterStatementSuffixMergeFiles
@init { pushMsg(retval.start,"", state); }
@after { popMsg(state); }
    : KW_CONCATENATE
    -> ^(TOK_ALTERTABLE_MERGEFILES)
    ;
...
alterStatementSuffixSetOwner
@init { pushMsg(retval.start,"alter table set owner", state); }
@after { popMsg(state); }
...
fileFormat
@init { pushMsg(retval.start,"file format specification", state); }
@after { popMsg(state); }
...
inputFileFormat
@init { pushMsg(retval.start,"Load Data input file format specification", state); }
@after { popMsg(state); }
...
tabTypeExpr
@init { pushMsg(retval.start,"specifying table types", state); }
@after { popMsg(state); }
...
partTypeExpr
@init { pushMsg(retval.start,"specifying table partitions", state); }
@after { popMsg(state); }
    :  tabTypeExpr partitionSpec? -> ^(TOK_TABTYPE tabTypeExpr partitionSpec?)
    ;

tabPartColTypeExpr
@init { pushMsg(retval.start,"specifying table partitions columnName", state); }
@after { popMsg(state); }
    :  tableName partitionSpec? extColumnName? -> ^(TOK_TABTYPE tableName partitionSpec? extColumnName?)
    ;

descStatement
@init { pushMsg(retval.start,"describe statement", state); }
@after { popMsg(state); }
    :
    (KW_DESCRIBE|KW_DESC)
    (
...
analyzeStatement
@init { pushMsg(retval.start,"analyze statement", state); }
@after { popMsg(state); }
...
showStatement
@init { pushMsg(retval.start,"show statement", state); }
@after { popMsg(state); }
...
dropRoleStatement
@init {pushMsg(retval.start,"drop role", state);}
@after {popMsg(state);}
...
grantPrivileges
@init {pushMsg(retval.start,"grant privileges", state);}
@after {popMsg(state);}
...
revokePrivileges
@init {pushMsg(retval.start,"revoke privileges", state);}
@afer {popMsg(state);}
...
grantRole
@init {pushMsg(retval.start,"grant role", state);}
@after {popMsg(state);}
...
revokeRole
@init {pushMsg(retval.start,"revoke role", state);}
@after {popMsg(state);}
...
showRoleGrants
@init {pushMsg(retval.start,"show role grants", state);}
@after {popMsg(state);}
...
showRoles
@init {pushMsg(retval.start,"show roles", state);}
@after {popMsg(state);}
    : KW_SHOW KW_ROLES
    -> ^(TOK_SHOW_ROLES)
    ;
...
showCurrentRole
@init {pushMsg(retval.start,"show current role", state);}
@after {popMsg(state);}
...
setRole
@init {pushMsg(retval.start,"set role", state);}
@after {popMsg(state);}
    : KW_SET KW_ROLE
    (
...
showGrants
@init {pushMsg(retval.start,"show grants", state);}
@after {popMsg(state);}
...
showRolePrincipals
@init {pushMsg(retval.start,"show role principals", state);}
@after {popMsg(state);}
...
@init {pushMsg(retval.start,"privilege object including columns", state);}
@after {popMsg(state);}
...
privilegeObject
@init {pushMsg(retval.start,"privilege object", state);}
@after {popMsg(state);}
...
privilegeList
@init {pushMsg(retval.start,"grant privilege list", state);}
@after {popMsg(state);}
...
privlegeDef
@init {pushMsg(retval.start,"grant privilege", state);}
@after {popMsg(state);}
...
privilegeType
@init {pushMsg(retval.start,"privilege type", state);}
@after {popMsg(state);}
    : KW_ALL -> ^(TOK_PRIV_ALL)
...
@init { pushMsg(retval.start,"user/group/role name list", state); }
@after { popMsg(state); }
    : principalName (COMMA principalName)* -> ^(TOK_PRINCIPAL_NAME principalName+)
    ;

principalName
@init {pushMsg(retval.start,"user|group|role name", state);}
@after {popMsg(state);}
...
withGrantOption
@init {pushMsg(retval.start,"with grant option", state);}
@after {popMsg(state);}
...
grantOptionFor
@init {pushMsg(retval.start,"grant option for", state);}
@after {popMsg(state);}
...
adminOptionFor
@init {pushMsg(retval.start,"admin option for", state);}
@after {popMsg(state);}
...
withAdminOption
@init {pushMsg(retval.start,"with admin option", state);}
@after {popMsg(state);}
...
metastoreCheck
@init { pushMsg(retval.start,"metastore check statement", state); }
@after { popMsg(state); }
...
resourceList
@init { pushMsg(retval.start,"resource list", state); }
@after { popMsg(state); }
  :
...
resource
@init { pushMsg(retval.start,"resource", state); }
@after { popMsg(state); }
  :
...
resourceType
@init { pushMsg(retval.start,"resource type", state); }
@after { popMsg(state); }
  :
  KW_JAR -> ^(TOK_JAR)
  |
...
createFunctionStatement
@init { pushMsg(retval.start,"create function statement", state); }
@after { popMsg(state); }
...
dropFunctionStatement
@init { pushMsg(retval.start,"drop function statement", state); }
@after { popMsg(state); }
...
reloadFunctionStatement
@init { pushMsg(retval.start,"reload function statement", state); }
@after { popMsg(state); }
    : KW_RELOAD KW_FUNCTION -> ^(TOK_RELOADFUNCTION);

createMacroStatement
@init { pushMsg(retval.start,"create macro statement", state); }
@after { popMsg(state); }
...
dropMacroStatement
@init { pushMsg(retval.start,"drop macro statement", state); }
@after { popMsg(state); }
...
viewPartition
@init { pushMsg(retval.start,"view partition specification", state); }
@after { popMsg(state); }
...
dropViewStatement
@init { pushMsg(retval.start,"drop view statement", state); }
@after { popMsg(state); }
...
dropMaterializedViewStatement
@init { pushMsg(retval.start,"drop materialized view statement", state); }
@after { popMsg(state); }
    : KW_DROP KW_MATERIALIZED KW_VIEW ifExists? viewName -> ^(TOK_DROP_MATERIALIZED_VIEW viewName ifExists?)
    ;

showFunctionIdentifier
@init { pushMsg(retval.start,"identifier for show function statement", state); }
@after { popMsg(state); }
    : functionIdentifier
    | StringLiteral
    ;
...
showStmtIdentifier
@init { pushMsg(retval.start,"identifier for show statement", state); }
@after { popMsg(state); }
    : identifier
    | StringLiteral
    ;
...
tableComment
@init { pushMsg(retval.start,"table's comment", state); }
@after { popMsg(state); }
    :
...
createTablePartitionSpec
@init { pushMsg(retval.start,"create table partition specification", state); }
@after { popMsg(state); }
...
tableBuckets
@init { pushMsg(retval.start,"table buckets specification", state); }
@after { popMsg(state); }
    :
...
tableSkewed
@init { pushMsg(retval.start,"table skewed specification", state); }
@after { popMsg(state); }
    :
...
rowFormat
@init { pushMsg(retval.start,"serde specification", state); }
@after { popMsg(state); }
...
recordReader
@init { pushMsg(retval.start,"record reader specification", state); }
@after { popMsg(state); }
...
recordWriter
@init { pushMsg(retval.start,"record writer specification", state); }
@after { popMsg(state); }
...
rowFormatSerde
@init { pushMsg(retval.start,"serde format specification", state); }
@after { popMsg(state); }
...
rowFormatDelimited
@init { pushMsg(retval.start,"serde properties specification", state); }
@after { popMsg(state); }
    :
...
tableRowFormat
@init { pushMsg(retval.start,"table row format specification", state); }
@after { popMsg(state); }
    :
      rowFormatDelimited
...
tablePropertiesPrefixed
@init { pushMsg(retval.start,"table properties with prefix", state); }
@after { popMsg(state); }
    :
...
tableProperties
@init { pushMsg(retval.start,"table properties", state); }
@after { popMsg(state); }
    :
...
tablePropertiesList
@init { pushMsg(retval.start,"table properties list", state); }
@after { popMsg(state); }
    :
...
keyValueProperty
@init { pushMsg(retval.start,"specifying key/value property", state); }
@after { popMsg(state); }
    :
...
keyProperty
@init { pushMsg(retval.start,"specifying key property", state); }
@after { popMsg(state); }
    :
...
@init { pushMsg(retval.start,"table row format's field separator", state); }
@after { popMsg(state); }
    :
...
@init { pushMsg(retval.start,"table row format's column separator", state); }
@after { popMsg(state); }
    :
...
@init { pushMsg(retval.start,"table row format's map key separator", state); }
@after { popMsg(state); }
    :
...
@init { pushMsg(retval.start,"table row format's line separator", state); }
@after { popMsg(state); }
    :
...
tableRowNullFormat
@init { pushMsg(retval.start,"table row format's null specifier", state); }
@after { popMsg(state); }
    :
    ...
    ;
tableFileFormat
@init { pushMsg(retval.start,"table file format specification", state); }
@after { popMsg(state); }
    :
...
tableLocation
@init { pushMsg(retval.start,"table location specification", state); }
@after { popMsg(state); }
    :
...
columnNameTypeList
@init { pushMsg(retval.start,"column name type list", state); }
@after { popMsg(state); }
    : columnNameType (COMMA columnNameType)* -> ^(TOK_TABCOLLIST columnNameType+)
    ;

columnNameTypeOrConstraintList
@init { pushMsg(retval.start,"column name type and constraints list", state); }
@after { popMsg(state); }
...
columnNameList
@init { pushMsg(retval.start,"column name list", state); }
@after { popMsg(state); }
    : columnName (COMMA columnName)* -> ^(TOK_TABCOLNAME columnName+)
    ;

columnName
@init { pushMsg(retval.start,"column name", state); }
@after { popMsg(state); }
    :
      identifier
    ;
...
extColumnName
@init { pushMsg(retval.start,"column name for complex types", state); }
@after { popMsg(state); }
    :
...
columnNameOrderList
@init { pushMsg(retval.start,"column name order list", state); }
@after { popMsg(state); }
    : columnNameOrder (COMMA columnNameOrder)* -> ^(TOK_TABCOLNAME columnNameOrder+)
    ;

columnParenthesesList
@init { pushMsg(retval.start,"column parentheses list", state); }
@after { popMsg(state); }
    : LPAREN! columnNameList RPAREN!
    ;

enableValidateSpecification
@init { pushMsg(retval.start,"enable specification", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"validate specification", state); }
@after { popMsg(state); }
...
enforcedSpecification
@init { pushMsg(retval.start,"enforced specification", state); }
@after { popMsg(state); }
...
relySpecification
@init { pushMsg(retval.start,"rely specification", state); }
@after { popMsg(state); }
    :  KW_RELY -> ^(TOK_RELY)
...
createConstraint
@init { pushMsg(retval.start,"pk or uk or nn constraint", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"pk or uk or nn constraint with name", state); }
@after { popMsg(state); }
...
pkUkConstraint
@init { pushMsg(retval.start,"pk or uk table level constraint", state); }
@after { popMsg(state); }
...
checkConstraint
@init { pushMsg(retval.start,"CHECK constraint", state); }
@after { popMsg(state); }
    : KW_CHECK expression
...
createForeignKey
@init { pushMsg(retval.start,"foreign key", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"foreign key with key name", state); }
@after { popMsg(state); }
...
skewedValueElement
@init { pushMsg(retval.start,"skewed value element", state); }
@after { popMsg(state); }
    :
      skewedColumnValues
...
skewedColumnValuePairList
@init { pushMsg(retval.start,"column value pair list", state); }
@after { popMsg(state); }
    : skewedColumnValuePair (COMMA skewedColumnValuePair)* -> ^(TOK_TABCOLVALUE_PAIR skewedColumnValuePair+)
    ;

skewedColumnValuePair
@init { pushMsg(retval.start,"column value pair", state); }
@after { popMsg(state); }
    :
...
skewedColumnValues
@init { pushMsg(retval.start,"column values", state); }
@after { popMsg(state); }
    : skewedColumnValue (COMMA skewedColumnValue)* -> ^(TOK_TABCOLVALUE skewedColumnValue+)
    ;

skewedColumnValue
@init { pushMsg(retval.start,"column value", state); }
@after { popMsg(state); }
    :
      constant
    ;
...
@init { pushMsg(retval.start,"skewed value location element", state); }
@after { popMsg(state); }
    :
      skewedColumnValue
     | skewedColumnValuePair
...
orderSpecification
@init { pushMsg(retval.start,"order specification", state); }
@after { popMsg(state); }
    : KW_ASC | KW_DESC ;

nullOrdering
@init { pushMsg(retval.start,"nulls ordering", state); }
@after { popMsg(state); }
...
columnNameOrder
@init { pushMsg(retval.start,"column name order", state); }
@after { popMsg(state); }
...
columnNameCommentList
@init { pushMsg(retval.start,"column name comment list", state); }
@after { popMsg(state); }
    : columnNameComment (COMMA columnNameComment)* -> ^(TOK_TABCOLNAME columnNameComment+)
    ;

columnNameComment
@init { pushMsg(retval.start,"column name comment", state); }
@after { popMsg(state); }
...
columnRefOrder
@init { pushMsg(retval.start,"column order", state); }
@after { popMsg(state); }
...
columnNameType
@init { pushMsg(retval.start,"column specification", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"column name or constraint", state); }
@after { popMsg(state); }
    : ( tableConstraint )
...
tableConstraint
@init { pushMsg(retval.start,"table constraint", state); }
@after { popMsg(state); }
    : ( createForeignKey )
    | ( createConstraint )
    ;
...
@init { pushMsg(retval.start,"column constraint", state); }
@after { popMsg(state); }
...
@init { pushMsg(retval.start,"alter column constraint", state); }
@after { popMsg(state); }
...
colType
@init { pushMsg(retval.start,"column type", state); }
@after { popMsg(state); }
    : type
    ;

colTypeList
@init { pushMsg(retval.start,"column type list", state); }
@after { popMsg(state); }
...
primitiveType
@init { pushMsg(retval.start,"primitive type specification", state); }
@after { popMsg(state); }
...
unionType
@init { pushMsg(retval.start,"uniontype type", state); }
@after { popMsg(state); }
    : KW_UNIONTYPE LESSTHAN colTypeList GREATERTHAN -> ^(TOK_UNIONTYPE colTypeList)
    ;

setOperator
@init { pushMsg(retval.start,"set operator", state); }
@after { popMsg(state); }
...
insertClause
@init { pushMsg(retval.start,"insert clause", state); }
@after { popMsg(state); }
   :
...
destination
@init { pushMsg(retval.start,"destination specification", state); }
@after { popMsg(state); }
   :
...
limitClause
@init { pushMsg(retval.start,"limit clause", state); }
@after { popMsg(state); }
   :
...
deleteStatement
@init { pushMsg(retval.start,"delete statement", state); }
@after { popMsg(state); }
   :
...
*/
updateStatement
@init { pushMsg(retval.start,"update statement", state); }
@after { popMsg(state); }
   :
...
*/
sqlTransactionStatement
@init { pushMsg(retval.start,"transaction statement", state); }
@after { popMsg(state); }
  :
...
abortTransactionStatement
@init { pushMsg(retval.start,"abort transactions statement", state); }
@after { popMsg(state); }
  :
...
*/
mergeStatement
@init { pushMsg(retval.start,"MERGE statement", state); }
@after { popMsg(state); }
   :
...
@init { pushMsg(retval.start,"WHEN NOT MATCHED clause", state); }
@after { popMsg(state); }
   :
...
@init { pushMsg(retval.start,"WHEN MATCHED AND clause", state); }
@after { popMsg(state); }
  :
...
@init { pushMsg(retval.start,"WHEN MATCHED THEN clause", state); }
@after { popMsg(state); }
  :
...
killQueryStatement
@init { pushMsg(retval.start,"kill query statement", state); }
@after { popMsg(state); }
  :
