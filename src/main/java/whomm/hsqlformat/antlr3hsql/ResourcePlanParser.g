...
rpAssign
@init { gParent.pushMsg(retval.start,"rpAssign", state); }
...
rpAssignList
@init { gParent.pushMsg(retval.start,"rpAssignList", state); }
@after { gParent.popMsg(state); }
...
createResourcePlanStatement
@init { gParent.pushMsg(retval.start,"create resource plan statement", state); }
@after { gParent.popMsg(state); }
...
alterResourcePlanStatement
@init { gParent.pushMsg(retval.start,"alter resource plan statement", state); }
@after { gParent.popMsg(state); }
...
globalWmStatement
@init { gParent.pushMsg(retval.start,"global WM statement", state); }
@after { gParent.popMsg(state); }
    : (enable | disable) KW_WORKLOAD KW_MANAGEMENT -> ^(TOK_ALTER_RP enable? disable?)
    ;

replaceResourcePlanStatement
@init { gParent.pushMsg(retval.start,"replace resource plan statement", state); }
@after { gParent.popMsg(state); }
    : KW_REPLACE (
...
dropResourcePlanStatement
@init { gParent.pushMsg(retval.start,"drop resource plan statement", state); }
@after { gParent.popMsg(state); }
    : KW_DROP KW_RESOURCE KW_PLAN ifExists? name=identifier -> ^(TOK_DROP_RP $name ifExists?)
    ;

poolPath
@init { gParent.pushMsg(retval.start,"poolPath", state); }
@after { gParent.popMsg(state); }
    : identifier^ (DOT identifier)*
    ;

triggerExpression
@init { gParent.pushMsg(retval.start,"triggerExpression", state); }
@after { gParent.popMsg(state); }
...
*/
triggerOrExpression
@init { gParent.pushMsg(retval.start,"triggerOrExpression", state); }
@after { gParent.popMsg(state); }
    : triggerAndExpression (KW_OR triggerAndExpression)*
    ;

triggerAndExpression
@init { gParent.pushMsg(retval.start,"triggerAndExpression", state); }
@after { gParent.popMsg(state); }
    : triggerAtomExpression (KW_AND triggerAtomExpression)*
    ;

triggerAtomExpression
@init { gParent.pushMsg(retval.start,"triggerAtomExpression", state); }
@after { gParent.popMsg(state); }
...
triggerLiteral
@init { gParent.pushMsg(retval.start,"triggerLiteral", state); }
@after { gParent.popMsg(state); }
    : Number
    | StringLiteral
    ;
...
comparisionOperator
@init { gParent.pushMsg(retval.start,"comparisionOperator", state); }
@after { gParent.popMsg(state); }
    : GREATERTHAN
    ;

triggerActionExpression
@init { gParent.pushMsg(retval.start,"triggerActionExpression", state); }
@after { gParent.popMsg(state); }
    : KW_KILL
...
createTriggerStatement
@init { gParent.pushMsg(retval.start,"create trigger statement", state); }
@after { gParent.popMsg(state); }
...
alterTriggerStatement
@init { gParent.pushMsg(retval.start,"alter trigger statement", state); }
@after { gParent.popMsg(state); }
...
dropTriggerStatement
@init { gParent.pushMsg(retval.start,"drop trigger statement", state); }
@after { gParent.popMsg(state); }
...
poolAssign
@init { gParent.pushMsg(retval.start,"poolAssign", state); }
@after { gParent.popMsg(state); }
    : (
...
poolAssignList
@init { gParent.pushMsg(retval.start,"poolAssignList", state); }
@after { gParent.popMsg(state); }
    : poolAssign (COMMA poolAssign)* -> poolAssign+
    ;

createPoolStatement
@init { gParent.pushMsg(retval.start,"create pool statement", state); }
@after { gParent.popMsg(state); }
...
alterPoolStatement
@init { gParent.pushMsg(retval.start,"alter pool statement", state); }
@after { gParent.popMsg(state); }
...
dropPoolStatement
@init { gParent.pushMsg(retval.start,"drop pool statement", state); }
@after { gParent.popMsg(state); }
...
createMappingStatement
@init { gParent.pushMsg(retval.start,"create mapping statement", state); }
@after { gParent.popMsg(state); }
...
alterMappingStatement
@init { gParent.pushMsg(retval.start,"alter mapping statement", state); }
@after { gParent.popMsg(state); }
...
dropMappingStatement
@init { gParent.pushMsg(retval.start,"drop mapping statement", state); }
@after { gParent.popMsg(state); }
