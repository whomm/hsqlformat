...
// group by a,b
groupByClause
@init { gParent.pushMsg(retval.start,"group by clause", state); }
...
rollupStandard
@init { gParent.pushMsg(retval.start,"standard rollup syntax", state); }
@after { gParent.popMsg(state); }
    :
...
rollupOldSyntax
@init { gParent.pushMsg(retval.start,"rollup old syntax", state); }
@after { gParent.popMsg(state); }
    :
...
@init {gParent.pushMsg(retval.start,"grouping set expression", state); }
@after {gParent.popMsg(state); }
   :
...
@init {gParent.pushMsg(retval.start,"grouping set part expression", state); }
@after {gParent.popMsg(state); }
   :
   LPAREN
...
groupingExpressionSingle
@init { gParent.pushMsg(retval.start,"groupingExpression expression", state); }
@after { gParent.popMsg(state); }
    :
...
havingClause
@init { gParent.pushMsg(retval.start,"having clause", state); }
@after { gParent.popMsg(state); }
    :
...
havingCondition
@init { gParent.pushMsg(retval.start,"having condition", state); }
@after { gParent.popMsg(state); }
    :
    expression
    ;
...
// order by a,b
orderByClause
@init { gParent.pushMsg(retval.start,"order by clause", state); }
@after { gParent.popMsg(state); }
    :
...
clusterByClause
@init { gParent.pushMsg(retval.start,"cluster by clause", state); }
@after { gParent.popMsg(state); }
    :
...
partitionByClause
@init  { gParent.pushMsg(retval.start,"partition by clause", state); }
@after { gParent.popMsg(state); }
    :
...
distributeByClause
@init { gParent.pushMsg(retval.start,"distribute by clause", state); }
@after { gParent.popMsg(state); }
    :
...
sortByClause
@init { gParent.pushMsg(retval.start,"sort by clause", state); }
@after { gParent.popMsg(state); }
    :
    KW_SORT KW_BY
    (
...
// fun(par1, par2, par3)
function
@init { gParent.pushMsg(retval.start,"function specification", state); }
@after { gParent.popMsg(state); }
    :
    functionName
    LPAREN
...
functionName
@init { gParent.pushMsg(retval.start,"function name", state); }
@after { gParent.popMsg(state); }
...
castExpression
@init { gParent.pushMsg(retval.start,"cast expression", state); }
@after { gParent.popMsg(state); }
    :
    KW_CAST
    LPAREN
...
caseExpression
@init { gParent.pushMsg(retval.start,"case expression", state); }
@after { gParent.popMsg(state); }
    :
    KW_CASE expression
...
constant
@init { gParent.pushMsg(retval.start,"constant", state); }
@after { gParent.popMsg(state); }
    :
...
charSetStringLiteral
@init { gParent.pushMsg(retval.start,"character string literal", state); }
@after { gParent.popMsg(state); }
    :
...
expression
@init { gParent.pushMsg(retval.start,"expression specification", state); }
@after { gParent.popMsg(state); }
    :
...
functionIdentifier
@init { gParent.pushMsg(retval.start,"function identifier", state); }
@after { gParent.popMsg(state); }
...
principalIdentifier
@init { gParent.pushMsg(retval.start,"identifier for principal spec", state); }
@after { gParent.popMsg(state); }
    : identifier
    | QuotedIdentifier
    ;
...
nonReserved
    ...
    | KW_UNKNOWN
    | KW_DATE  //这个地方添加个date  order group  这样 date 字符可以直接用作别名 不用转义
    | KW_ORDER 
    | KW_GROUP
;
